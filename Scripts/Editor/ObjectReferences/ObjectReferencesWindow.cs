...
namespace HJ.Editors
{
    public class ObjectReferencesWindow : EditorWindow
    {
        ...
        private ObjectReferences _target;

        [SerializeField]
        private MultiColumnHeaderState _objRefColumnState;
        private ReferencesHeader _objRefColumn;
        private Vector2 _columnScrollPosition;

        private SearchField _searchField;
        private string _searchString;
        private Color _dragDropColor;

        private List<ObjectReferenceElement> _objRefArray;
        ...
        public void Init(ObjectReferences target)
        {
            ...
            _objRefColumnState = CreateMultiColumnHeaderState();
            _objRefColumn = new ReferencesHeader(_objRefColumnState, position.width - 10f)
            {
        ...
        {
            _objRefArray = new List<ObjectReferenceElement>();
            foreach (var kv in _target.References)
            {
                string path = AssetDatabase.GetAssetPath(kv.Object);
                _objRefArray.Add(new ObjectReferenceElement()
                {
                    Obj = kv.Object,
                    Path = path,
                    GUID = kv.GUID
        ...
        private void OnGUI()
        {
            ...
            _objRefColumn.maxHeaderWidth = rect.width;
            ...
            GUILayout.BeginArea(rect);
            {
                ...
                using (new EditorGUI.DisabledGroupScope(_target.References.Count <= 0))
                {
                ...
                Rect columnHeader = GUILayoutUtility.GetRect(1, _objRefColumn.height);
                _objRefColumn.OnGUI(columnHeader, 0);

                ObjectReferenceElement[] SearchResult = GetSearchResult(_searchString);
                _columnScrollPosition = EditorGUILayout.BeginScrollView(_columnScrollPosition);
                {
                    Event e = Event.current;

                    foreach (var elm in SearchResult)
                    {
                        int i = _objRefArray.IndexOf(elm);
                        ...
                        for (int j = 0; j < _objRefColumnState.columns.Length; j++)
                        {
                            int index = _objRefColumn.GetVisibleColumnIndex(j);
                            Rect cellRect = _objRefColumn.GetCellRect(index, rowRect);
        ...
        private ObjectReferenceElement[] GetSearchResult(string search)
        {
            if (!string.IsNullOrEmpty(search))
            {
                ...
                return (from reference in _objRefArray
            ...
            return _objRefArray.ToArray();
        }

        private void OnColumnRowGUI(Rect rect, int column, int index)
        {
            ObjectReferenceElement element = _objRefArray[index];
        ...
        private void DragDropAreaGUI()
        {
            ...
            switch (evt.type)
            {
                ...
                case EventType.DragPerform:
                    ...
                    if (CanAcceptDrag(objects, paths))
                    {
                        ...
                        _dragDropColor = new Color(1, 1, 1, 0.05f);
            ...
            EditorGUI.DrawRect(dropAreaRect, _dragDropColor);
        }
        ...
        private void AddObjectToTarget(string path)
        {
            if (AssetDatabase.IsValidFolder(path))
            {
                ...
                foreach (string pathGuid in assets)
                {
                    ...
                    if (!_target.References.Any(x => x.GUID == pathGuid))
                    {
                        _target.References.Add(new ObjectReferences.ObjectGuidPair()
                        {
                            Object = obj,
                            GUID = pathGuid
                        });
            ...
            else
            {
                ...
                if (asset.GetType() == typeof(GameObject))
                {
                    ...
                    if (!_target.References.Any(x => x.GUID == guid))
                    {
                        _target.References.Add(new ObjectReferences.ObjectGuidPair()
                        {
                            Object = obj,
